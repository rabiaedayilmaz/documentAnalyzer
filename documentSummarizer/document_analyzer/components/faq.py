...
def faq():
    st.markdown(
        """
# FAQ
## How does DocumentAnalyzer work?
...
When you ask a question, DocumentAnalyzer will search through the
document chunks and find the most relevant ones using the vector index.
Then, it will use GPT-3 to generate a final answer.
...
No, the answers are not 100% accurate. DocumentAnalyzer uses GPT-3 to generate
answers. GPT-3 is a powerful language model, but it sometimes makes mistakes 
and is prone to hallucinations. Also, DocumentAnalyzer uses semantic search
...
But for most use cases, DocumentAnalyzer is very accurate and can answer
